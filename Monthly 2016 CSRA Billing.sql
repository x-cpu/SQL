...
select distinct x.UploadDate, ISNULL(sum(x.FaxDocs), 0) FaxDocs, ISNULL(sum(x.FaxImages),0) FaxImages, ISNULL(sum(x.PaperDocs), 0) PaperDocs, ISNULL(sum(x.PaperImages), 0) PaperImages, Y.CharCounts DCSCharCounts
FROM
...
on s.dcsID = c.dcsID
where s.ackTime is not null and s.ackTime >= '2016-03-01 0:0:0'
group by convert(nvarchar, s.ackTime, 101), s.batchname) X,
(select convert(nvarchar, s.acktime, 101) UploadDate, 
sum((len(c.Recvdate) - (len(c.Recvdate) - len(replace(c.Recvdate, '/', '')))) + len(c.FileNumber) + len(c.Lastname) + len(c.Firstname) + len(c.MiddleInitial)) CharCounts 
from smsDCSMapping s
left join CustomerDATA c
on s.dcsid = c.dcsID
where s.ackTime is not null and s.ackTime >= '2016-03-01 0:0:0'
...
sum((len(Form526Data) - (len(Form526Data) - LEN(replace(form526data, ' ', '')))) - (len(Form526Data) - LEN(replace(form526data, ';', '')))) "526 Character Count" from smsDCSMapping s
left join Document d
on s.dcsid = d.dcsID
where s.ackTime is not null and d.docidDoctype like 'VA 21-526%'  and s.ackTime >= '2016-03-01 0:0:0'
