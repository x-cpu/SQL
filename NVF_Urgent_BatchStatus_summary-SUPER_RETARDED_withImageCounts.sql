...
sum(CASE
	...
	Else 0
	END) as ACKd, ISNULL(sum(Z.TotalImages),0) TotalImages
FROM
...
CASE 
	...
	END BatchStatus,
CASE
	When (select count(*) from smsDCSMapping where batchname = X.BatchName and acktime is not null) > 0 Then ISNULL(sum(d.numpages),0)
	Else 0
	END TotalImages
FROM
...
on n.BatchName = d.batchname) X
left join document d
on X.BatchName = d.PBatch
--left join smsDCSMapping s
--on X.BatchName = s.batchname
group by X.BatchName, X.RMN, X.ExportedBatchName, X.DownloadedBatchName, X.UploadedBatchName, X.TurboscanBatchName, X.TurboscanBatchLocation, X.BatchStatus) Z
group by z.RMN