--VTG data pull 
select distinct Z.FileNumber, UPPER(Z.Firstname) FirstName, UPPER(Z.MiddleInitial) MiddleInitial, 
REPLACE(UPPER(Z.Lastname), char(9), ' ') LastName,
...
CASE
	...
	ELSE null
	END FAXno,
Z.ImageID, Z.NumPages Images, RIGHT('00000'+ISNULL(d.docid,''), 5) DocID, 
Z.Form526Data Contentions, 
Z.received_date ReceivedDate, Z.ackTime [DMHS ack date],
CASE
	WHEN Z.status = 'COMPLETED' then Z.xmlModified
	ELSE NULL
	END [VBMS ack date], Z.UUID
FROM
(select distinct Y.PacketID, Y.BatchName, Y.dcsid, Y.ackTime, Y.xmlModified, Y.status, Y.UUID,
...
v.RoSiteID
FROM
(select distinct X.PacketID, X.BatchName, X.dcsid, X.ackTime, X.xmlModified, X.status, X.UUID,
...
FROM
(select distinct T.PacketID, T.BatchName, T.dcsid, T.ackTime, T.xmlModified, T.status, T.UUID,
...
s.ackTime, s.xmlModified, s.status, d.UUID
from smsDCSMapping s
left join DCSID_UUID_mapping d
on s.dcsid = d.DCSID
where s.ackTime >= '2017-01-01' and s.ackTime < '2017-05-01' 
