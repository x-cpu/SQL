...
sum(CASE
	When (Z.BatchStatus <> 'Not Available' and Z.BatchStatus not like 'Exported%') Then 1
	Else 0
	END) as InProcess,
sum(CASE
	When (Z.BatchStatus like 'Exported%') Then 1
	Else 0
	END) as Exported,
sum(CASE
...
select * from TURBOSCANNG1.dbo.batches b where exists
(select * from PhysicalBatch where rmn = 'C20076566' and b.BatchName = pbatch)
order by BatchName
