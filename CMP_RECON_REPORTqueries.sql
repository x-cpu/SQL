--latest as of 3-18-2015
select T.kbatch BatchName, T.exportdate ExportDate, T.Status, T.#ofIMGs, substring(f.comments, 45, 40) PortalError, max(f.etime) LastPortalUploadAttempt
FROM
(select distinct s.kbatch, max(s.releasedate) as exportdate, 
case
	when s.kbatch not in (select distinct batchname from FTPDetails) then 'OCR IN PROGRESS'
	when s.kbatch in (select distinct batchname from FTPDetails where ETime is not null and AckTime is null and comments is not null 
and batchname not in (select distinct batchname from ftpdetails where acktime is not null))  then 'ERR RECEIVED FROM PORTAL'
	Else 'ACK NOT RECEIVED FROM PORTAL'
END
as Status, sum(s.images) #ofIMGs 
from stats s
where (s.kbatch like '03%' or s.kbatch like '04%' or s.kbatch like '05%' )
and not exists (select * from cmprecon where cmprecon.pBatch = s.kbatch)
and not exists (select * from FTPDetails where FTPDetails.batchname = s.kbatch and AckTime is not null)
group by s.kbatch) T
left outer join FTPDetails f
on T.kbatch = f.batchname
group by T.kbatch, T.exportdate, t.Status, t.#ofIMGs, f.comments
order by max(t.exportdate)




--latest as of 2015-03-17
select distinct s.kbatch, max(s.releasedate) as exportdate, 
case
	when s.kbatch not in (select distinct batchname from FTPDetails) then 'OCR IN PROGRESS'
	when s.kbatch in (select distinct batchname from FTPDetails where ETime is not null and AckTime is null and comments is not null 
...
and not exists (select * from FTPDetails where FTPDetails.batchname = s.kbatch and AckTime is not null)
group by s.kbatch
order by max(s.releasedate)



---latest as of 2015-03-05
select s.kbatch, max(s.releasedate) as exportdate, f.comments 
from stats s 
left join FTPDetails f
on s.kbatch = f.batchname
where (s.kbatch like '03%' or s.kbatch like '04%' or s.kbatch like '05%' )
and s.kbatch not in (select distinct batchname from ftpdetails where AckTime is not null)
and s.kbatch not in (select distinct pBatch from cmprecon where s.kbatch = cmprecon.pBatch)
and s.kbatch not in (select distinct batchname from FTPDetails where etime is not null and comments is null)
group by s.kbatch, f.comments 
order by max(s.releasedate)


---latest as of 2015-03-05
select count(distinct s.kbatch)
from stats s 
left join FTPDetails f
on s.kbatch = f.batchname
where (s.kbatch like '03%' or s.kbatch like '04%' or s.kbatch like '05%' )
...
select * from FTPDetails where batchname = '03150750637101'
select * from FTPDetails order by etime desc
...
select * from FTPDetails where batchname = '03150720621801'
select 
...
select * from cmprecon where pbatch = '03150750637101'