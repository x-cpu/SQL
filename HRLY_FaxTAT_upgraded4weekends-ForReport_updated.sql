...
(ISNULL(sum(Z.Faxes),0) - (ISNULL(sum(Z.[5Hrs]), 0) + ISNULL(sum(Z.[6Hrs]), 0) + ISNULL(sum(Z.[12Hrs]), 0) + ISNULL(sum(Z.[24Hrs]), 0) + ISNULL(sum(Z.[48Hrs]), 0)  + ISNULL(sum(Z.More48Hrs), 0))) WIP,
...
CASE
	When Y.TotalHours <=5 Then Y.Faxes
	END '5Hrs',
CASE
