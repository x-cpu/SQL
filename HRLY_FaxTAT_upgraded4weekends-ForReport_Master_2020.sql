...
ISNULL(sum(Z.More48Hrs), 0) 'More48Hrs', AVG(Z.TotalHours) AverageHrs
FROM
...
CASE
	...
	When DATEPART(dw, T.IncomingDate) = 6 and DATEPART(dw, T.UploadDate) = 2 Then DATEDIFF(hour, T.IncomingDate, T.UploadDate) - 
	DATEDIFF(hour, CONVERT(varchar(10), T.IncomingDate, 120) + ' 22:00:00', CONVERT(varchar(10), T.NextBusinessDay, 120) + ' 06:00:00')
